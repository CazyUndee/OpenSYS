...
### Near-term
- **IN PROGRESS (2026-08-25, mid-session): part.c partitioned disk I/O** — code COMPLETE + host-tested (156/156), QEMU validation PENDING. Done: `part.c` rewritten (part_ready flag, real `part_get_info` from GPT entries incl. UTF-16 label→ASCII + type classification, dense `part_list_partitions` skipping unused slots, `part_translate` partition-relative LBA + bounds checks [sectors 1..256, range within entry, LBA28 cap] used by `part_read/write_sectors`, `part_get_disk_info` via `disk_get_info`); `part.h` declares `part_is_ready()`; `kernel.c` calls `part_init()` after disk_init success (non-fatal without GPT); shell `show partitions` + `parts` alias (`cmd_parts`: no-disk/no-GPT guards, lists #/start-LBA/size-KB/label); mocks added to `tests/mocks/mock_kernel.c` (un-prefixed disk_init/disk_is_ready/disk_read/disk_write/disk_get_info/disk_get_size + gpt_init/gpt_list_partitions/gpt_get_partition + setters mock_disk_set_ready/mock_disk_io_reset/mock_disk_last_io/mock_gpt_setup, MOCK_GPT_MAX_PARTS=16); `tests/unit/test_part.c` 8 tests (translation 10→2058, write→2048, bounds rejections never touch disk, requires-ready, get_info fields, dense list); Makefile REAL_SRCS+=src/drivers/part.c UNIT_TEST_SRCS+=test_part.c; test_runner registered "Partition Manager". Kernel build clean; host suite 156/156. REMAINING STEPS: (1) write `tools/drive_parts.py` modeled on `tools/drive_vcat.py` harness (QEMU spawn `-accel whpx -display none -serial file:LOG -monitor tcp:127.0.0.1:4451 -m 128 -kernel bin/kernel0.bin` PLUS `-drive file=tools/gpt_test.img,if=ide,format=raw,index=0`; sendkey/type_text/LogWatcher echo-verified pattern; port 4451); checks: serial log contains "[PART] Partition Management Ready!" + "Found 2 partitions"; type `parts\n` → expect rows "2048", "1024", "plan0-test" and "4096"/"data-two"; regression `wc /proc/uptime`. NOTE fs_mount reads LBA0 → protective MBR → magic mismatch → in-memory format path (fs_format writes boot sector over MBR — disposable image, regenerate each run). (2) run `python tools/make_gpt_image.py` then `drive_parts.py` until green. (3) HISTORY.md entry + strike this line. (4) Commit granularly: part.c/part.h/kernel.c/shell.c one commit ("Implement partition-relative sector I/O..."), tests+mocks second, tools third. Image generator `tools/make_gpt_image.py` already written (16 MiB, protective MBR, header LBA1 entry_count=4 array@LBA2, p1 "plan0-test" LINUX_FS GUID 2048-4095, p2 "data-two" FAT32 GUID 4096-6143; CRCs zeroed — kernel has GPT_VALIDATE_CRC=0).
