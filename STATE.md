...
5. **fs.c is not partition-aware**: GPT parsing and partition-relative sector I/O (`part_read/write_sectors`) work, but `fs_format`/`fs_mount` still use whole-disk LBA 0 — on a GPT disk the fs boot sector lands over the protective MBR and the fs spans the whole device. Wiring fs.c behind a chosen partition (mount fs at partition 1 start) is the remaining integration step for true partitioned persistence.
