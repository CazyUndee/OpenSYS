...
/*
 * Stack frame layout matches boot/interrupts.asm isr_common:
 *   push order: rax, rbx, rcx, rdx, rsi, rdi, rbp, r8, r9, r10, r11, r12, r13, r14, r15
 *   then int_no (stub), err_code (stub or CPU)
 *   then CPU-pushed: RIP, CS, RFLAGS [, RSP, SS if from user mode]
 */
typedef struct {
    ...
    /* RSP and SS only present if interrupt came from user mode (ring 3) */
    uint64_t rsp, ss;
