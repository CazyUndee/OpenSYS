...
/* Array of page tables (physical before recursion, recursive after paging) */
static pte_t* page_tables[PD_ENTRIES];

/* Recursive mapping becomes usable after paging is enabled */
static uint8_t paging_recursive_ready = 0;

static inline pte_t* get_pt(uint32_t pd_index) {
    if (pd_index >= PD_ENTRIES) return 0;
    if (!(kernel_pd[pd_index] & PTE_PRESENT)) return 0;

    if (paging_recursive_ready) {
        return (pte_t*)(0xFFC00000 + pd_index * PAGE_SIZE);
    }
...
static pte_t* get_or_create_pt(uint32_t pd_index, uint32_t flags) {
    ...
    pte_t* pt = paging_recursive_ready
        ? (pte_t*)(0xFFC00000 + pd_index * PAGE_SIZE)
        : (pte_t*)pt_phys;
...
void paging_identity_map(uint32_t start, uint32_t end, uint32_t flags) {
    ...
    for (uint32_t addr = start; addr < end; addr += PAGE_SIZE) {
        ...
        pt[pt_index] = addr | flags | PTE_PRESENT;
        invlpg(addr);
    }
}
...
void paging_init(void) {
    ...
    /* Clear page directory and page table tracking */
    for (int i = 0; i < PD_ENTRIES; i++) {
        kernel_pd[i] = 0;
        page_tables[i] = 0;
    }
    
    /* Build the initial identity map directly with physical addresses. */
